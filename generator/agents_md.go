...
func (g *AgentsMdGenerator) Generate(ctx GenerateContext) (map[string]string, error) {
	...
	sb.WriteString("This file was scaffolded by proser. To flesh it out, ask your AI agent:\n\n")
	sb.WriteString("> Analyze this repository and expand AGENTS.md following the PROSE\n")
	sb.WriteString("> Explicit Hierarchy convention. Document the project structure,\n")
	sb.WriteString("> key modules, architectural decisions, and domain-specific guidelines.\n")
	sb.WriteString("> Keep it concise — context is a scarce resource.\n\n")
	sb.WriteString("If you have distinct project directories (e.g. `client/` and `server/`),\n")
	sb.WriteString("create an `AGENTS.md` in each with domain-specific guidance.\n\n")
